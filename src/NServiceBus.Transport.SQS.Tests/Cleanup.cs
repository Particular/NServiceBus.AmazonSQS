...
public class Cleanup
{
    ...
    [Test]
    [Explicit]
    public async Task DeleteAllBucketsUsedForTests() => await DeleteAllBucketsWithPrefix(s3Client, "cli-");
    ...
    public static Task DeleteAllResourcesWithPrefix(IAmazonSQS sqsClient, IAmazonSimpleNotificationService snsClient,
        IAmazonS3 s3Client, string namePrefix) =>
        Task.WhenAll(
    ...
    public static async Task DeleteAllBucketsWithPrefix(IAmazonS3 s3Client, string namePrefix)
    {
        ListBucketsResponse listBucketsResponse =
            await s3Client.ListBucketsAsync(new ListBucketsRequest()).ConfigureAwait(false);

        await Task.WhenAll(listBucketsResponse.Buckets
            .Where(x => x.BucketName.StartsWith(namePrefix, StringComparison.OrdinalIgnoreCase))
            .Select(x => x.BucketName).Select(async bucketName =>
            {
                try
                {
                    ...
                    GetBucketLocationResponse response = await s3Client.GetBucketLocationAsync(bucketName);
                    S3Region region;
    ...
    public static async Task DeleteAllSubscriptionsWithPrefix(IAmazonSimpleNotificationService snsClient,
        string topicNamePrefix)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                if (subscriptions.Subscriptions.All(subscription =>
                        deletedSubscriptionArns.Contains(subscription.SubscriptionArn)))
                {
                    return;
                }
                ...
                deletionTasks.AddRange(subscriptions.Subscriptions
                    ...
                    .Select(async subscription =>
                    {
                        ...
                        {
                            Console.WriteLine(
                                $"Unable to delete subscription '{subscription.SubscriptionArn}' '{subscription.TopicArn}'");
                        }
                    }));
    ...
    public static async Task DeleteAllTopicsWithPrefix(IAmazonSimpleNotificationService snsClient,
        string topicNamePrefix)
    {
