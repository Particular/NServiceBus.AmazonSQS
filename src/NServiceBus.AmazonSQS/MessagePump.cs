namespace NServiceBus.Transports.SQS
{
    ...
    class MessagePump : IPushMessages
    {
        ...
        List<ReceivedDelayedMessageAsPreparedMessage> PrepareMessages(CancellationToken token, ReceiveMessageResponse receivedMessages, TimeSpan clockCorrection)
        {
            List<ReceivedDelayedMessageAsPreparedMessage> preparedMessages = null;
            foreach (var receivedMessage in receivedMessages.Messages)
            {
                token.ThrowIfCancellationRequested();

                preparedMessages = preparedMessages ?? new List<ReceivedDelayedMessageAsPreparedMessage>(receivedMessages.Messages.Count);
                long delaySeconds = 0;
                ...
                ReceivedDelayedMessageAsPreparedMessage preparedMessage;

                if (remainingDelay > configuration.DelayedDeliveryQueueDelayTime)
                {
                    preparedMessage = new ReceivedDelayedMessageAsPreparedMessage(originalMessageId, receivedMessage.ReceiptHandle)
                    {
                        QueueUrl = delayedDeliveryQueueUrl,
                        MessageAttributes =
                        {
        ...
        async Task BatchDispatchPreparedMessages(List<ReceivedDelayedMessageAsPreparedMessage> preparedMessages)
        {
        ...
        async Task SendDelayedMessagesInBatches(BatchEntry<ReceivedDelayedMessageAsPreparedMessage> batch, int batchNumber, int totalBatches)
        {
        ...
        async Task ChangeVisibilityOfDelayedMessagesInBatches(BatchEntry<ReceivedDelayedMessageAsPreparedMessage> batch, SendMessageBatchResponse result, int batchNumber, int totalBatches)
        {
        ...
        async Task DeleteDelayedMessagesInBatches(BatchEntry<ReceivedDelayedMessageAsPreparedMessage> batch, SendMessageBatchResponse result, int batchNumber, int totalBatches)
        {
