...
static class TestNameHelper
{
    public static string GetSqsQueueName(string destination, string queueNamePrefix)
    {
        ...
        string queueName = !string.IsNullOrEmpty(queueNamePrefix) &&
        ...
        {
            int charsToTake = 80 - queueNamePrefix.Length;
    ...
    static string GetSanitizedQueueName(string queueName)
    {
        ...
        int skipCharacters = queueName.EndsWith(".fifo") ? 5 : 0;
        // SQS queue names can only have alphanumeric characters, hyphens and underscores.
        // Any other characters will be replaced with a hyphen.
        for (int i = 0; i < queueNameBuilder.Length - skipCharacters; ++i)
        {
    ...
    public static string GetSnsTopicName(Type eventType, string topicNamePrefix)
    {
        ...
        string s = topicNamePrefix + destination;

        if (s.Length > 256)
        {
            int charsToTake = 256 - topicNamePrefix.Length;
            s = topicNamePrefix +
        ...
        {
            throw new Exception(
                $"Address {destination} with configured prefix {topicNamePrefix} is longer than 256 characters and therefore cannot be used to create an SNS topic. Use a shorter topic name.");
        }
    ...
    public static string GetSanitizedTopicName(StringBuilder topicNameBuilder)
    {
        ...
        // Any other characters will be replaced with a hyphen.
        for (int i = 0; i < topicNameBuilder.Length; ++i)
        {
            char c = topicNameBuilder[i];
            if (!char.IsLetterOrDigit(c)
                && c != '-'
                && c != '_')
            {
